...
export function openPrompt(options:any) {
	...
	let mouseOutside = (e:MouseEvent) => { if (!wrapper.contains(e.target as (Node|null))) close() }
	...
	domFields.forEach( (field:Element) => {
	...
	let input:HTMLElement = form.elements[0] as HTMLElement;
...
function reportInvalid(dom:HTMLElement, message:string) {
	...
	let parent:HTMLElement = dom.parentNode as HTMLElement;
...
export class Field<T=any> {
	...
	validateType(_value:any):(string|undefined) { return undefined; }
