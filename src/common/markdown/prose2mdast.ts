...
= (type: string) => (
	...
	if(children[0].children !== undefined) { throw new Error("[prose2mdast] expected leaf literal node"); }
	if(!children[0].value)                 { throw new Error("[prose2mdast] expected non-empty literal node"); }
 ...
 * Unist `Literal` nodes.  
 */
export const markMapLiteral: (type:string) => ProseMarkMap
= (type: string) => (
	mark: ProseMark,
	node: Uni.Node
): Uni.Node => {
	...
	if(node.value === undefined) { throw new Error(`[prose2mdast] expected Unist.Literal node when wrapping with mark '${mark.type.name}'`); }
