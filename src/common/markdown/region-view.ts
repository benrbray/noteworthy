...
export class RegionView implements NodeView {
	// nodeview params
	...
	constructor(node: ProseNode, view: EditorView, getPos: (() => number), mainProxy: MainIpcHandlers){
		...
		// file provided?
		let regionName: string | null = node.attrs.region || null;
		if(regionName){
			regionName = replaceInvalidFileNameChars(regionName);
			console.log("region-view :: name provided ::", regionName);
			this.setEmbedTag(regionName);
		}
